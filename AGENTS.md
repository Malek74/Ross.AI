...
Clauses + GOAL ("audit this contract under <domain> law; produce a cited risk report")
    ...
    ▼
        ...
        tools:
          • search_statutes(query)  → top-k articles from THIS domain's index (AR+EN)
...
| Labour *or* Commercial | **STRETCH — pick one** | dataflare, filtered by `law_name` |
...
REGISTRY = {
  ...
  "labour":     DomainAgent("labour",     "data/index/labour",     "playbooks/labour.yaml"),  # if built
  "commercial": StubAgent("commercial"),
...
```
data/index/
├── civil/         # TawasulAI articles → FAISS + metadata
├── labour/        # dataflare filtered on law_name → chunked → cleaned → FAISS
...
- **Retrieval:** dense multilingual embeddings → FAISS. **Drop the TF-IDF path** in the source repo (weak on Arabic).
...
| `graphrag/` (builder, store, retriever, schema) | **Keep as stretch** | Only wire up if core lands early (article cross-reference edges). |
...
│   │   └── graph_interface.py       # keep (stretch)
...
**Two-pane audit view — not a plain chat clone.**
